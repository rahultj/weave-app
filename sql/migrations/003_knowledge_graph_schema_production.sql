...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_artifacts_discovered_via'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_connections_discovered_in_conversation'
  ) THEN
...
SELECT 'Knowledge graph schema migration complete!' as status,
       ...
       'Skipped data migration (clean install)' as migration_status;
